...
// ── Moved OUTSIDE the component so they are never recreated on re-render ──────
// This prevents inputs from losing focus after each keystroke.
...
function inputCls(err) {
  return `h-10 w-full rounded-md border bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
...
export default function EmployeeForm() {
  ...
  useEffect(() => {
    ...
    api.get('/employees?page_size=100').then((r) => {
      const mgrs = r.data.items.filter(e => e.role === 'senior_manager' || e.role === 'management_admin');
      setManagers(mgrs);
    }).catch(() => {});
  ...
  const uploadPhoto = async (employeeId) => {
    ...
    try {
      ...
      // Photo upload is optional — employee was already created
      // Show a warning but don't block navigation
      console.warn('Photo upload failed:', photoErr);
      toast('Employee created, but photo upload failed. You can upload it later from the profile.', { icon: '⚠️', duration: 5000 });
    }
  };
  ...
  const submit = async (e) => {
    ...
    try {
      if (isEdit) {
        ...
        // uploadPhoto handles its own errors — won't block navigation
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Employee code" error={errors.employee_code}>
              <input
                ...
                onChange={(e) => upd('employee_code', e.target.value)}
                data-testid="form-employee-code"
              />
            </Field>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Designation">
            <input
              ...
              data-testid="form-designation"
            />
          </Field>

          {form.role !== 'senior_manager' && form.role !== 'management_admin' && (
            <Field label="Manager">
              <select
                className={inputCls(false)}
              ...
              >
                <option value="">— None —</option>
                {managers.map((m) => (
                  <option key={m.id} value={m.id}>{m.full_name} ({m.designation || 'Manager'})</option>
                ))}
              </select>
            </Field>
          )}
